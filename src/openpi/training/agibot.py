...
AgiBot ships no success/failure field, so every kept subtask is marked success. This mirrors
`robomind.record_from_annotation`'s structure (explicit start/end spans) rather than robocoin's RLE.
The result is the SAME record shape (id, goal, subtasks, frame_ranges, success_flags) as
`robomind.RobomindRecord`, so `robomind.build_samples` and the memory-label generator reuse it
unchanged.
...
def record_from_episode(repo: str, task_id, entry: dict) -> AgibotRecord | None:
    ...
    for c in action_config:
        ...
        ranges.append((int(c["start_frame"]), int(c["end_frame"]) - 1))  # exclusive-end -> inclusive
    if not subtasks:
        return None
    return AgibotRecord(
